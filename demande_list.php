...
/**
 *   	\file       demande_list.php
 *		\ingroup    repliclient
 *		\brief      List page for demande
 */
...
require_once __DIR__.'/class/demande.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('repliclient', 'demande', 'write');
	$permissiontodelete = $user->hasRight('repliclient', 'demande', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("repliclient")) {
	accessforbidden('Module repliclient not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Demande';
	$objectlabel = 'Demande';
	$uploaddir = $conf->repliclient->dir_output;
...
$title = $langs->trans("Demandes");
//$help_url = "EN:Module_Demande|FR:Module_Demande_FR|ES:Módulo_Demande";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/repliclient/demande_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-repliclient page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/repliclient/demande_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDemandeRef";
$modelmail = "demande";
$objecttmp = new Demande($db);
