...
/**
 *   	\file       suivit_list.php
 *		\ingroup    repliclient
 *		\brief      List page for suivit
 */
...
require_once __DIR__.'/class/suivit.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('repliclient', 'suivit', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
$title = $langs->trans("Suivits");
//$help_url = "EN:Module_Suivit|FR:Module_Suivit_FR|ES:Módulo_Suivit";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/repliclient/suivit_card.php', 1).'?id='.((int) $id));
	exit;
}
...
// $newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/repliclient/suivit_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendSuivitRef";
$modelmail = "suivit";
$objecttmp = new Suivit($db);
	...
	if ($mode == 'kanban') {
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				} elseif($key == 'conterendu'){
					// $chaine= strlen($object->$key);
					print substr($object->$key, 0, 97);
				
					if(strlen($object->$key) > 97 )	{print "...";}
				}
				 else {
