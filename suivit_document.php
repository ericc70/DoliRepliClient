...
/**
 *  \file       suivit_document.php
 *  \ingroup    repliclient
 *  \brief      Tab for documents linked to Suivit
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->repliclient->multidir_output[$object->entity ? $object->entity : $conf->entity]."/suivit/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('repliclient', 'suivit', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Suivit")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Suivit"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='suivit/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'suivit/'.dol_sanitizeFileName($object->ref).'/';
